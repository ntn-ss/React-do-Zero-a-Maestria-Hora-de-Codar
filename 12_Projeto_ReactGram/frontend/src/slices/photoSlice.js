...
export const comment = createAsyncThunk("photo/comment", async(commentData, thunkAPI)=>{
  const token = thunkAPI.getState().auth.user.token
  const data = await photoService.addComment({comment: commentData.comment}, commentData.id, token)
...
// get all photos
// o _ serve para dizer ao Reac que um argumento é desnecessário.
